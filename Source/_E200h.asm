...
; Первый КД тут гасить нельзя: у второго КД то же окно A000-DFFF, и оно
; подменяет ОЗУ, где у программы лежит буфер обмена. Для первого КД
; отключение и не нужно -- следом в тот же порт уходит конфиг.
	NOP		; было: XRA A
	NOP		; было: OUT 010h -- отключение первого КД
	NOP
