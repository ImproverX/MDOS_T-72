...
;
	...
	#DEFINE D_HDDD 0FFCAh	; количество дискет НЖМД, если 0 -- нет НЖМД
;
...
L_E221:	DCR  A		;MVI  A, 003h	; // отключение второго КД
...
#endif
L__HDD: SHLD	D_HDDD	; конфигурация для CO
	MOV  A, H	; инверсия HL
	CMA
	MOV  H, A
	MOV  A, L
...
L_E37Y:	LHLD    L_E995	; адрес контрольной суммы сектора КД
	...
	MOV  A, E
	CMP  D
	JNZ     L_E37E	; КС считана с ошибкой
	CMP  B
	JZ      L_E38A	; >> всё ок
...
;
L_E37E: CMP  B
	JZ      L_E388	; Е совпало с расчётным КС, исправляем Д
	MOV  A, D
	CMP  B
	MOV  E, D
	JZ      L_E389	; Д совпало, исправляем Е
	MVI  C, 003h	; всё плохо...
	JMP     L_E38A
L_E388: MOV  D, E
...
; вход:		L_E98E -- номер дорожки КД, L_E98F -- номер сектора КД, L_E98A -- номер диска
; выход:	L_E994 -- конфиг.КД, L_E992 -- адрес на КД,
;		L_E995 -- адрес контрольной суммы сектора КД
;
L_E3DB:	LDA     L_E98A  ; считываем номер диска
	SUI	003h	; КД 11 ?
	JZ	L_E3DZ
	MVI  A, 023h	; 0010 0011	ОЗУ
L_E3DZ:	ORI	010h	; 0001 0000	СТЕК
	STA	LxE3EZ+1
...
L_E3E7: SUB  H		; A = номер_дорожки - H
	...
	RLC
	ANI     00Ch	; 0000 1100
LxE3EZ:	ORI     010h	; 0001 0000 // 033h	; 0011 0011	
