...
L_BEXX:	PUSH D
	...
	LXI  D, L_BEFF	; параметр вызова БДОС	
...
#else
	PUSH D		; адрес БУФ
	LXI  H, L_FSUB	; перенос ПП -- что переносим
	LXI  D, M_BC00-(L_FEND-L_FSUB)-1	; куда
	MVI  C, L_FEND-L_FSUB
LaMOVE:	MOV  A, M
	STAX D
	INX  H
	...
	JNZ     LaMOVE
	POP  D
	JMP     M_BC00-(L_FEND-L_FSUB)-1	; запуск L_FSUB после переноса
;
L_FSUB:	MVI  C, 011h	; Поиск первого файла (флоповод)	<<< переносится в ХХХХ..BC00
	CALL    00005h	;					<<<
	INR  A		; А<>0 -- нашли				<<<
	LXI  D, M_BC00-(L_FEND-L_BEFF)-1	; дескриптор	<<< переносится следом за кодом
#endif
L_ISUB:	MVI  C, 032h	; исполнение операций расширенной БДОС	<
