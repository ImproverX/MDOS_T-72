...
L_BEXX:	PUSH D
	...
	INR  A		; А = 1 -- номер диска А:
	CMP  M		; на нём уже искали?
	JZ      L_BE9C	; да -- больше искать негде
...
#ifdef NoHDD
	JMP     L_ASUB	; А: -- дисковод, отсюда искать на нём нельзя
#else
...
;
#ifdef NoHDD
; Искать INITIAL.SUB на дискете отсюда нельзя: БСВВ читает её каталог через
; килобайтный буфер по D_FD_B (0BC00h), а он накрывает всю ССР, 0BE00h..0BFFFh.
; Код, который ведёт поиск, стирается собственным же чтением, управление уходит
; в байты 0E5h -- это PUSH H, дальше бесконечный PUSH и разнесённая память.
; Поэтому поиск и выход переносим в ОЗУ пользователя и доделываем оттуда.
L_ASUB:	LXI  H, L_FSUB	; что переносим
	LXI  D, 00100h	; куда -- начало ОЗУ пользователя
	MVI  C, L_FEND-L_FSUB
LaMOVE:	MOV  A, M
	STAX D
	INX  H
	...
	JMP     00100h
;
; Этот кусок исполняется по 00100h, поэтому своих меток в нём быть не может:
; все адреса абсолютные, а условный переход заменён условным вызовом.
L_FSUB:	LXI  D, M_E049	; ФУБ, уже переписанный на A:INITIAL.SUB
	MVI  C, 011h	; Поиск первого файла
	CALL    00005h	; <<<<<<
	INR  A		; А<>0 -- нашли
	LXI  D, 00100h+LaDESC-L_FSUB	; дескриптор -- переносится следом за кодом
	MVI  C, 032h	; исполнение операций расширенной БДОС
	CNZ     00005h	; <<<<<< только если нашли
	MVI  A, 0FFh
	STA     L_DE80
	JMP     L_DA4D	; >>>> запуск МДОС
LaDESC:	.db 083h	; номер операции расширенной БДОС
